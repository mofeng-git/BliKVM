<template>
  ...
  >
    ...
    >
      ...
      <img :src="redDotCursor" alt="cursor" class="cursor-dot" />

      <!-- Label -->
      <v-chip variant="elevated" color="#D32F2F">
        {{ label }}
      </v-chip>
    </div>
  </v-overlay>
...
<script setup>
  ...
  const redDotCursor = computed(() => {
    // Simple red circle SVG converted to base64
    const svg = `<svg width="12" height="12" xmlns="http://www.w3.org/2000/svg">
      <circle cx="6" cy="6" r="5" fill="#D32F2F" stroke="#004400" stroke-width="1"/>
    </svg>`;
